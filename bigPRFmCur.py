...
colors = ['r', 'g', 'b', 'y', 'c', 'm', 'k', 'orange', 'purple', 'brown', 'pink', 'lightblue', 'lime', 'gold', 'navy',
          'teal', 'coral', 'violet']
linestyles = ['-', '--']  # 添加虚线样式
...
for i, (method, (Rs, Ps)) in enumerate(pr_vals.items()):
    linewidth = 2
    if method == 'TCAINet':
        linewidth = 4  # 增大 TCAINet 的线条宽度以增加对比度
    plt.plot(Rs, Ps, linestyle=linestyles[i % len(linestyles)], color=colors[i % len(colors)], linewidth=linewidth,
             label=method)
...
# 添加放大的插图到右上角
ax = plt.gca()
axins = ax.inset_axes([0.5, 0.5, 0.4, 0.4])  # 调整位置到右上角
for i, (method, (Rs, Ps)) in enumerate(pr_vals.items()):
    linewidth = 2
    if method == 'TCAINet':
        linewidth = 4
    axins.plot(Rs, Ps, linestyle=linestyles[i % len(linestyles)], color=colors[i % len(colors)], linewidth=linewidth)
x1, x2, y1, y2 = 0.8, 1, 0.8, 1  # 关键区域
...
plt.savefig('TCAINETPR821.png', bbox_inches='tight',dpi=300)
plt.show()
...
for i, (method, F1) in enumerate(fmeasure_vals.items()):
    ...
    if method == 'TCAINet':
        linewidth = 4  # 增大 TCAINet 的线条宽度以增加对比度
    plt.plot(bins[:-1] / 255.0, F1, linestyle=linestyle, color=color, linewidth=linewidth, label=method)

plt.grid(True)
plt.title(f'F-measure Curve for VT5000', fontsize=_font_size_ + 2)
plt.xlim([0.0, 1.0])
...
for i, (method, F1) in enumerate(fmeasure_vals.items()):
    ...
    if method == 'TCAINet':
        linewidth = 4
    axins.plot(bins[:-1] / 255.0, F1, linestyle=linestyle, color=color, linewidth=linewidth)
x1, x2, y1, y2 = 0.8, 1, 0.8, 1  # 关键区域
axins.set_xlim(x1, x2)
axins.set_ylim(y1, y2)
axins.set_xticklabels([])
...
plt.savefig('TCAINETfmeasure5000.png', bbox_inches='tight',dpi=300)
plt.show()
