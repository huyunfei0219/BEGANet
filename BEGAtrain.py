...
parser.add_argument('--rgb_root', type=str, default='./test_maps/train_dut/train_dut/train_images/',
                    help='the training rgb images root')  # train_dut
parser.add_argument('--depth_root', type=str, default='./test_maps/train_dut/train_dut/train_depth/',
                    help='the training depth images root')
parser.add_argument('--gt_root', type=str, default='./test_maps/train_dut/train_dut/train_masks/',
                    help='the training gt images root')

parser.add_argument('--val_rgb', type=str, default="./test_maps/test_datasets/test_datasets/NLPR/RGB/",
                    help='validate rgb path')
parser.add_argument('--val_depth', type=str, default="./test_maps/test_datasets/test_datasets/NLPR/depth/",
                    help='validate depth path')
parser.add_argument('--val_gt', type=str, default="./test_maps/test_datasets/test_datasets/NLPR/GT/",
...
def train(train_loader, model, optimizer, epoch):
    ...
    if epoch_loss < best_loss:
        ...
        # 新增逻辑：从第150个epoch开始，每个epoch结束后保存模型
    if epoch >= 150:  # 从epoch=150开始（包含150）
        save_filename = opt.save_path + f'Net_rgb_d_epoch_{epoch}.pth'
        torch.save(model.state_dict(), save_filename,
                   _use_new_zipfile_serialization=False)
        print(f"Epoch {epoch}: 已保存模型到 {save_filename}")
        logging.info(f"Epoch {epoch}: 保存模型到 {save_filename}")
...
print("Let's go!")
for epoch in range(opt.epoc h +1):
